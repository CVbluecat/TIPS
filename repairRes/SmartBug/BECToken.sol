...
library SafeMath {
	...
	function batchTransfer(address[] _receivers, uint256 _value) whenNotPaused public returns(bool ){
	uint cnt = _receivers.length;
	uint256 amount = uint256(cnt) * _value;
	if(_value != 0){
	require(amount / _value == uint256(cnt));
	}
	if(uint256(cnt) != 0){
	require(amount / uint256(cnt) == _value);
	}

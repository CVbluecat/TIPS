...
library SafeMath {
	...
	if(! targetToken.transfer(_exchanger, _tokens)){
	throw;}
	...
	function balanceOfTarget(address _owner) view public returns(uint256 targetBalance){
	...
	function canExchangeNum() view public returns(uint256 ){
	if(canExchange()){
	...
	function canExchange() view public returns(bool ){
	...
	function newCrowdSale(string _projectName, string _tokenName, string _tokenSymbol, uint256 _cap) onlyOwner public {
	require(_cap > 0);
	projectName = _projectName;
	tokenName = _tokenName;
	tokenSymbol = _tokenSymbol;
	...
	function buyTokens(address beneficiary) whenNotPaused payable public {
	...
	if(! token.mint(beneficiary, ethAmount)){
	throw;}
	...
	function saleRatio() view public returns(uint256 ratio){
	if(cap == 0){
	return 0;
	}
	else{

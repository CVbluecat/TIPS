...
contract Token {
	...
	constructor(address _icoAddress, address _preIcoAddress, address _easyMineWalletAddress, address _bountyWalletAddress) public {
	...
	function setup(address _easyMineToken, address _withdrawalAddress) isOwner public {
	...
	function withdraw(uint256 requestedAmount) isOwner public returns(uint256 amount){
	...
	function maxPossibleWithdrawal() view public returns(uint256 ){
	...
	function () timedTransitions payable public {
	...
	function setup(address _easyMineToken, address _sys, address _reservationAddress, uint256 _minStartDelay, uint256 _maxDuration) isOwner atStage(Stages.Deployed) public {
	require(_easyMineToken != 0x0);
	require(_sys != 0x0);
	...
	function maxTokensSold() view public returns(uint256 ){
	uint256 total = 0;
	for(uint8 i = 0;i < priceThresholds.length;i++){
	total += priceThresholds[i].tokenCount;
	require(total >= priceThresholds[i].tokenCount);
	}
	return total;
	}
	function totalTokensSold() view public returns(uint256 ){
	uint256 total = 0;
	for(uint8 i = 0;i < priceThresholds.length;i++){
	...
	function scheduleStart(uint256 _startBlock) isOwner atStage(Stages.SetUp) public {
	require(_startBlock > block.number + minStartDelay);
	startBlock = _startBlock;
	endBlock = startBlock + maxDuration;
	stage = Stages.StartScheduled;
	}
	function updateStage() timedTransitions public returns(Stages ){
	return stage;
	}
	function buyTokens() isValidPayload timedTransitions atStage(Stages.Started) payable public {
	require(msg.value > 0);
	uint256 amountRemaining = msg.value;
	uint256 tokensToReceive = 0;
	for(uint8 i = 0;i < priceThresholds.length;i++){
	require(priceThresholds[i].tokenCount >= priceThresholds[i].tokensSold);
	...
	require(priceThresholds[i].tokensSold >= tokens);
	...
	function reserveTokens(uint256 tokenCount) isSys timedTransitions atStage(Stages.Started) public {
	...
	function cleanup() isOwner timedTransitions atStage(Stages.Ended) public {
	assert(owner.send(this.balance));
	}
	function finalize() private {
	stage = Stages.Ended;
	uint256 balance = easyMineToken.balanceOf(this);
	if(! easyMineToken.burn(balance)){
	throw;}
	IcoFinished(balance);
	}

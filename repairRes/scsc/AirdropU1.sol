...
contract ERC20 {
	...
	if(! token.transferFrom(msg.sender, _to[i], _amount)){
	throw;}
	}
	}
	...
	ERC20 token = ERC20(_token);
	for(uint256 i = 0;i < _to.length;++ i){
	if(! token.transfer(_to[i], _amount)){
	throw;}
	}
	}
	...
	ERC20 token = ERC20(_token);
	for(uint256 i = 0;i < _to.length;++ i){
	if(! token.transferFrom(msg.sender, _to[i], _amount[i])){
	throw;}
	}
	}
	...
	ERC20 token = ERC20(_token);
	for(uint256 i = 0;i < _to.length;++ i){
	if(! token.transfer(_to[i], _amount[i])){
	throw;}
	}
	}

...
contract OraclizeI {
	...
	_;}
	function () allowedBets payable public {
	...
	function __callback(bytes32 myid, string result) onlyOraclize public {
	logfolio(betid[myid]);
	logResult(result);
	...
	assert((sub(pot[_id], _prize) == sub(pot[_id], mul(money[_id], 2))) || (sub(pot[_id], _prize) == pot[_id]) || (sub(pot[_id], _prize) == sub(pot[_id], mul(money[_id], 3))) || (sub(pot[_id], _prize) == sub(pot[_id], add(money[_id], mul(div(money[_id], 10), 5)))) || (sub(pot[_id], _prize) == sub(pot[_id], add(money[_id], div(money[_id], 10)))));
	return true;
	}
	...
	function freeBet(address player) onlyByOperative public {
	freeTry[player] = true;
	}
	function changeOperative(address newOperative) onlyByOwner public {
	Operative = newOperative;
	}
	function changeFreeBetValue(uint newAmount) onlyByOwner public {
	freeBetValue = newAmount;
	}
	function rangeOfBets(uint newMinBet, uint newMaxBet) onlyByOwner public {
	min = newMinBet;
	max = newMaxBet;
	}
	function changeAccount(address newAccount) onlyByOwner public {
	companyAccount = newAccount;
	}
	function depositInPot() onlyByOwner payable public {
	deposit(msg.value);
	}
	function withdrawal(address account, uint amount) onlyByOwner public {
	account.transfer(amount);
	}
	function KillContract() onlyByOwner public {
	selfdestruct(owner);
	}

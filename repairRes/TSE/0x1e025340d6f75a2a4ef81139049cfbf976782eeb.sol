...
contract Crowdsale {
	...
	function getTokenAmount(uint256 weiAmount) view internal returns(uint256 ){
...
contract TokenVesting is Ownable {
	...
	function revoke(ERC20Basic token) onlyOwner public {
	require(revocable);
	...
	function releasableAmount(ERC20Basic token) view public returns(uint256 ){
	return vestedAmount(token).sub(released[token]);
	}
	function vestedAmount(ERC20Basic token) view public returns(uint256 ){
...
contract DividendToken is StandardToken , Ownable {
	...
	function getClaimableDividend(address _hodler) view public returns(uint256 claimableDividend){
	if(lastUpdate[_hodler] < lastDividendIncreaseDate){
	...
	function setTreasurer(address _treasurer, bool _active) onlyOwner public {
	isTreasurer[_treasurer] = _active;
	ChangedTreasurer(_treasurer, _active);
	}
	function requestUnclaimed() onlyOwner public {
	...
	function calcDividend(address _hodler, uint256 _totalSupply) view public returns(uint256 ){
...
contract IcoToken is MintableToken , PausableToken , DividendToken {
	...
	constructor() DividendToken public {
	paused = true;
	}
...
contract IcoCrowdsale is Crowdsale , Ownable {
	...
	_;}
	constructor(uint256 _startTime, uint256 _endTime, uint256 _rateChfPerEth, address _wallet, uint256 _confirmationPeriodDays, address _underwriter) Crowdsale(_startTime,_endTime,_rateChfPerEth,_wallet) public {
	...
	function setManager(address _manager, bool _active) onlyOwner public {
	isManager[_manager] = _active;
	ChangedManager(_manager, _active);
	}
	function blackListInvestor(address _investor, bool _active) onlyManager public {
	...
	function confirmPayment(uint256 _investmentId) onlyManager onlyConfirmPayment public {
	...
	function batchConfirmPayments(uint256[] _investmentIds) onlyManager onlyConfirmPayment public {
	...
	function unConfirmPayment(uint256 _investmentId) onlyManager onlyConfirmPayment public {
	...
	function batchMintTokenDirect(address[] _toList, uint256[] _tokenList) onlyOwner public {
	require(_toList.length == _tokenList.length);
	for(uint256 i;i < _toList.length;i = i.add(1)){
	mintTokenDirect(_toList[i], _tokenList[i]);
	}
	}
	function mintTokenDirect(address _to, uint256 _tokens) onlyOwner public {
	...
	function mintIcoEnablersTokens(address _to, uint256 _tokens) onlyOwner onlyNoneZero(_to,_tokens) public {
	require(icoEnablersTokensMinted.add(_tokens) <= ICO_ENABLERS_CAP);
	if(! token.mint(_to, _tokens)){
	throw;}
	icoEnablersTokensMinted = icoEnablersTokensMinted.add(_tokens);
	}
	function mintDevelopmentTeamTokens(address _to, uint256 _tokens) onlyOwner onlyNoneZero(_to,_tokens) public {
	...
	if(! token.mint(address(newVault), _tokens)){
	throw;}
	}
	function getVestingWalletLength() view public returns(uint256 ){
	return vestingWallets.length;
	}
	function finalizeConfirmationPeriod() onlyOwner onlyConfirmPayment public {
	confirmationPeriodOver = true;
	}
	function settleInvestment(uint256 _investmentId) onlyConfirmationOver public {
	...
	if(! token.mint(p.beneficiary, tokens)){
	throw;}
	if(p.weiAmount > 0){
	...
	function finalize() onlyUnderwriter onlyConfirmationOver public {
	Pausable(token).unpause();
	IcoToken(token).setTreasurer(this, false);
	if(! MintableToken(token).finishMinting()){
	throw;}
	...
	require(underwriter == msg.sender);

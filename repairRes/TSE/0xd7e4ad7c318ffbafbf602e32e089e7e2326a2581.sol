...
contract ERC20Basic {
	...
	function reclaimContract(address contractAddr) onlyOwner external {
	...
	constructor(uint256 _hardCap, uint256[] roundStarts, uint256[] roundEnds, uint256[] roundRates, uint256[] roundRatesBulk, uint256[] roundBulkThreshold) public {
	token = new DOCTToken();
	...
	require((roundStarts.length > 0) && (roundStarts.length == roundEnds.length) && (roundStarts.length == roundRates.length) && (roundStarts.length == roundRatesBulk.length) && (roundStarts.length == roundBulkThreshold.length));
	uint256 prevRoundEnd = now;
	rounds.length = roundStarts.length;
	for(uint i = 0;i < roundStarts.length;i++){
	rounds[i] = Round(roundStarts[i], roundEnds[i], roundRates[i], roundRatesBulk[i], roundBulkThreshold[i]);
	...
	function crowdsaleRunning() view public returns(bool ){

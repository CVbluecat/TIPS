...
library SafeMath {
	function add(uint256 x, uint256 y) view internal returns(uint256 ){
	uint256 z = x + y;
	assert((z >= x) && (z >= y));
	return z;
	}
	function sub(uint256 x, uint256 y) view internal returns(uint256 ){
	assert(x >= y);
	uint256 z = x - y;
	return z;
	}
	function mul(uint256 x, uint256 y) view internal returns(uint256 ){
	uint256 z = x * y;
	assert((x == 0) || (z / x == y));
	return z;
	}
	function parse(string s) view internal returns(uint256 ){
	...
	function allowance(address _owner, address _spender) onlyPayloadSize(2) view public returns(uint256 remaining){
	...
	_;}
	constructor(uint256 _fundingStartBlock, uint256 _fundingEndBlock, uint256 _roundTwoBlock, uint256 _roundThreeBlock, uint256 _roundFourBlock, address _admin1, address _admin2, address _tokenVendor, uint256 _ccReleaseBlock) payable public {
	...
	function transfer(address _to, uint256 _value) isFinalized isKycVerified(msg.sender) hasEnoughUnlockedTokens(msg.sender,_value) onlyPayloadSize(2) public returns(bool success){
	...
	function transferFrom(address _from, address _to, uint256 _value) isFinalized isKycVerified(msg.sender) hasEnoughUnlockedTokens(msg.sender,_value) onlyPayloadSize(3) public returns(bool success){
	...
	function getBalanceOf(address _owner) view public returns(uint256 _balance){
	return balances[_owner];
	}
	function getPurchaseById(string _id) view public returns(address _buyer, uint256 _tokenAmount, bool _active){
	_buyer = purchases[_id].buyer;
	_tokenAmount = purchases[_id].tokenAmount;
	_active = purchases[_id].active;
	}
	function getHolderCount() view public returns(uint256 _holderCount){
	return holders.length;
	}
	function getPurchaseCount() view public returns(uint256 _purchaseCount){
	return purchaseArray.length;
	}
	function getHolder(uint256 _index) view public returns(address _holder){
	...
	function createTokens() isFundraising payable external {
	...
	function addToKycTeam(address _teamMember) onlyOwner onlyPayloadSize(1) public {
	isKycTeam[_teamMember] = true;
	}
	function removeFromKycTeam(address _teamMember) onlyOwner onlyPayloadSize(1) public {
	isKycTeam[_teamMember] = false;
	}
	function unlockKyc(address _owner) onlyKycTeam external {
	...
	function refuseKyc(address _user) onlyKycTeam external {
	...
	function cancelDelivery(string _purchaseID) onlyKycTeam external {
	...
	function deliverTokens(address _to, uint256 _tokens, string _purchaseId, bool _btcBuyer) isFundraising onlyVendor external {
	require(_to != 0x0);
	require(_tokens > 0);
	...
	purchases[_purchaseId] = Purchase(_to, tokens, true);
	...
	function getCurrentDiscountRate() view private returns(uint256 currentDiscountRate){
	...
	function retrieveEth(uint256 _value, address _safe) minimumReached onlyOwner external {
	require(SafeMath.sub(this.balance, _value) >= allUnKycedEth);
	require(_safe != 0x0);
	_safe.transfer(_value);
	}
	function finalize(address _safe) isFundraising minimumReached onlyOwner external {
	...
	function proceed() isPaused onlyOwner external {
	state = savedState;
	}
	...
	function deliverTeamTokens(address _to) isFinalized onlyOwner external {
	...
	function safeMulPercentage(uint256 value, uint256 percentage) view internal returns(uint256 resultValue){
	...
	function setOraclizeGas(uint256 _option) onlyOwner external {
	if(_option <= 30){

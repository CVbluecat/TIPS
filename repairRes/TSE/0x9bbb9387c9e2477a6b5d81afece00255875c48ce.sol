...
library SafeMath {
	...
	function updateMaxGasPrice(uint256 _maxGasPrice) onlyOwner public {
	require(_maxGasPrice > 0);
	maxGasPrice = _maxGasPrice;
	}
	function getEtherContributed() onlyOwner view public returns(uint256 ){
	return etherContributed;
	}
	function getXNKPurchased() onlyOwner view public returns(uint256 ){
	return xnkPurchased;
	}
	function updateEtherCap(uint256 _newEtherCap) notFinalized onlyOwner public {
	etherCap = _newEtherCap;
	}
	function updateGlobalMax(uint256 _globalMax) notFinalized onlyOwner public {
	require(_globalMax > globalMin);
	globalMax = _globalMax;
	}
	function updateGlobalMin(uint256 _globalMin) notFinalized onlyOwner public {
	require(_globalMin > 0);
	require(_globalMin < globalMax);
	globalMin = _globalMin;
	}
	function updateTokenAddress(address _tokenAddress) finalized onlyOwner public {
	...
	function disableRefund() onlyOwner public {
	require(refundable);
	refundable = false;
	}
	function addContributor(address _account, uint256 _rate, uint256 _max) onlyOwner notFinalized public {
	...
	function updateContributor(address _account, uint256 _newRate, uint256 _newMax) onlyOwner notFinalized public {
	require(_account != address(0));
	require(_newRate > 0);
	...
	function removeContributor(address _account) onlyOwner public {
	require(_account != address(0));
	require(contributors[_account].whitelisted);
	...
	! _account.send(balance);
	}
	delete contributors[_account];
	}
	function withdrawXNK(address _to) onlyOwner public {
	require(_to != address(0));
	BasicToken token = BasicToken(tokenAddress);
	assert(token.transfer(_to, token.balanceOf(this)));
	}
	function withdrawEther(address _to) finalized onlyOwner public {
	require(_to != address(0));
	...
	function airdrop(address _account) finalized onlyOwner public {
	_processPayout(_account);
	}
	function finalize(address _tokenAddress) notFinalized onlyOwner public {
	require(_tokenAddress != address(0));
	tokenAddress = _tokenAddress;
	}

...
contract OraclizeI {
	...
	function query(uint _timestamp, string _datasource, string _arg) payable public returns(bytes32 _id);function query_withGasLimit(uint _timestamp, string _datasource, string _arg, uint _gaslimit) payable public returns(bytes32 _id);function query2(uint _timestamp, string _datasource, string _arg1, string _arg2) payable public returns(bytes32 _id);function query2_withGasLimit(uint _timestamp, string _datasource, string _arg1, string _arg2, uint _gaslimit) payable public returns(bytes32 _id);function queryN(uint _timestamp, string _datasource, bytes _argN) payable public returns(bytes32 _id);function queryN_withGasLimit(uint _timestamp, string _datasource, bytes _argN, uint _gaslimit) payable public returns(bytes32 _id);function getPrice(string _datasource) public returns(uint _dsprice);function getPrice(string _datasource, uint gaslimit) public returns(uint _dsprice);function useCoupon(string _coupon) public ;function setProofType(byte _proofType) public ;function setConfig(bytes32 _config) public ;function setCustomGasPrice(uint _gasPrice) public ;function randomDS_getSessionPubKeyHash() public returns(bytes32 );
...
contract DSMath {
	...
	function mul(uint256 x, uint256 y) view internal returns(uint256 z){
	z = x * y;
	assert(x == 0 || z / x == y);
	}
	function div(uint256 x, uint256 y) view internal returns(uint256 z){
	z = x / y;
	}
	function min(uint256 x, uint256 y) view internal returns(uint256 z){
	return x <= y?x:y;
	}
	function max(uint256 x, uint256 y) view internal returns(uint256 z){
	return x >= y?x:y;
	}
	function hadd(uint128 x, uint128 y) view internal returns(uint128 z){
	assert((z = x + y) >= x);
	}
	function hsub(uint128 x, uint128 y) view internal returns(uint128 z){
	assert((z = x - y) <= x);
	}
	function hmul(uint128 x, uint128 y) view internal returns(uint128 z){
	z = x * y;
	assert(x == 0 || z / x == y);
	}
	function hdiv(uint128 x, uint128 y) view internal returns(uint128 z){
	z = x / y;
	}
	function hmin(uint128 x, uint128 y) view internal returns(uint128 z){
	return x <= y?x:y;
	}
	function hmax(uint128 x, uint128 y) view internal returns(uint128 z){
	return x >= y?x:y;
	}
	function imin(int256 x, int256 y) view internal returns(int256 z){
	return x <= y?x:y;
	}
	function imax(int256 x, int256 y) view internal returns(int256 z){
	return x >= y?x:y;
	}
	uint128 constant WAD = 10 ** 18;
	function wadd(uint128 x, uint128 y) view internal returns(uint128 ){
	return hadd(x, y);
	}
	function wsub(uint128 x, uint128 y) view internal returns(uint128 ){
	return hsub(x, y);
	}
	...
	function wmin(uint128 x, uint128 y) view internal returns(uint128 ){
	return hmin(x, y);
	}
	function wmax(uint128 x, uint128 y) view internal returns(uint128 ){
	return hmax(x, y);
	}
	uint128 constant RAY = 10 ** 27;
	function radd(uint128 x, uint128 y) view internal returns(uint128 ){
	return hadd(x, y);
	}
	function rsub(uint128 x, uint128 y) view internal returns(uint128 ){
	return hsub(x, y);
	}
	...
	function rmin(uint128 x, uint128 y) view internal returns(uint128 ){
	return hmin(x, y);
	}
	function rmax(uint128 x, uint128 y) view internal returns(uint128 ){
	return hmax(x, y);
	}
	function cast(uint256 x) view internal returns(uint128 z){
	assert((z = uint128(x)) == x);
	}
	
}contract LedgerProofVerifyI {
	function external_oraclize_randomDS_setCommitment(bytes32 queryId, bytes32 commitment) public ;function external_oraclize_randomDS_proofVerify(bytes proof, bytes32 queryId, bytes result, string context_name) public returns(bool );
}contract Owned {
	...
	function setSafeGas(uint _gas) onlyOwner public {
	...
	function stop_or_resume_Contract(bool _isStopped) onlyOwner public {
	isStopped = _isStopped;
	}
	function changeHouse(address _newHouse) onlyOwner public {
	...
	_;assert(numInvestors <= MAX_INVESTORS);
	}
	function getBankroll() view public returns(uint ){
	if((invested < investorsProfit) || (invested + investorsProfit < invested) || (invested + investorsProfit < investorsLosses)){
	return 0;
	}
	else{
	...
	function getMinInvestment() view public returns(uint ){
	...
	function getLossesShare(address currentInvestor) view public returns(uint ){
	return (investors[investorIDs[currentInvestor]].amountInvested * investorsLosses) / invested;
	}
	function getProfitShare(address currentInvestor) view public returns(uint ){
	return (investors[investorIDs[currentInvestor]].amountInvested * investorsProfit) / invested;
	}
	function getBalance(address currentInvestor) view public returns(uint ){
	...
	if((invested + profit < profit) || (invested + profit < invested) || (invested + profit < losses)){
	return 0;
	}
	else{
	...
	function searchSmallestInvestor() view public returns(uint ){
	uint investorID = 1;
	...
	function profitDistribution() private {
	...
	function increaseInvestment() onlyIfNotStopped onlyMoreThanZero onlyInvestors payable public {
	profitDistribution();
	investors[investorIDs[msg.sender]].amountInvested += msg.value;
	invested += msg.value;
	}
	function newInvestor() onlyIfNotStopped onlyMoreThanZero onlyNotInvestors onlyMoreThanMinInvestment investorsInvariant payable public {
	profitDistribution();
	if(numInvestors == MAX_INVESTORS){
	...
	function divest() onlyInvestors public {
	divest(msg.sender);
	}
	function divest(address currentInvestor) investorsInvariant internal {
	profitDistribution();
	...
	function forceDivestOfAllInvestors() onlyOwner public {
	...
	_;}
	function voteEmergencyWithdrawal(bool vote) onlyInvestors onlyAfterProposed onlyIfStopped public {
	investors[investorIDs[msg.sender]].votedForEmergencyWithdrawal = vote;
	LOG_EmergencyWithdrawalVote(msg.sender, vote);
	}
	function proposeEmergencyWithdrawal(address withdrawalAddress) onlyIfStopped onlyOwner public {
	for(uint i = 1;i <= numInvestors;i++){
	...
	function executeEmergencyWithdrawal() onlyOwner onlyAfterProposed onlyIfStopped onlyIfEmergencyTimeOutHasPassed public {
	...
	function forceDivestOfOneInvestor(address currentInvestor) onlyOwner onlyIfStopped public {
	...
	constructor(address _verifierAddr, address _owner) public {
	...
	_;}
	function isValidSize(uint _amountWagered) internal returns(bool ){
	...
	if(_prob.length != _payouts.length){
	revert();
	}
	uint sum = 0;
	...
	function buySpins(uint _nSpins) onlyValidNumberOfSpins(_nSpins) onlyIfNotStopped payable public {
	...
	function executeSpins(bytes32 myid, bytes randomBytes) private returns(uint ){
	...
	function __callback(bytes32 myid, string result, bytes _proof) onlyOraclize onlyIfSpinsExist(myid) onlyIfEnoughFunds(myid) oraclize_randomDS_proofVerify(myid,result,_proof) public {
	uint payout = executeSpins(myid, bytes(result));
	sendPayout(myid, payout);
	delete profitDistributed;
	delete spins[myid];
	}
	function setConfiguration(uint[] _probabilities, uint[] _multipliers) onlyOwner onlyIfFair(_probabilities,_multipliers) public {
	...
	if(lastProbability >= _probabilities[i]){
	revert();
	}
	...
	if(lastMultiplier <= _multipliers[i]){
	revert();
	}
	lastMultiplier = _multipliers[i];
	}
	}
	function setMinBet(uint _minBet) onlyOwner public {
	minBet = _minBet;
	}
	function getSpinsContainer(bytes32 myid) view public returns(address , uint ){
	return (spins[myid].playerAddress,spins[myid].amountWagered);
	}
	function getMinAmountToWager(uint _nSpins) onlyValidNumberOfSpins(_nSpins) view public returns(uint ){
	uint gas = _nSpins * ORACLIZE_PER_SPIN_GAS_LIMIT + ORACLIZE_BASE_GAS_LIMIT + safeGas;
	uint oraclizeFee = OraclizeI(OAR.getAddress()).getPrice("random", gas);
	return minBet + oraclizeFee / multipliers[0] + oraclizeFee;
	}
	function getMaxAmountToWager(uint _nSpins) onlyValidNumberOfSpins(_nSpins) view public returns(uint ){

...
library SafeMath {
	...
	function isCurrentAccountReader(address _address) view public returns(bool ){
	return accountReaderAddresses[_address];
	}
	function isCurrentOrPastAccountReader(address _address) view public returns(bool ){
	...
	function isCurrentAccountMinter(address _address) view public returns(bool ){
	return accountMinterAddresses[_address];
	}
	function isCurrentOrPastAccountMinter(address _address) view public returns(bool ){
	...
	function addAccountMinter(address _address) public {
	if(! isCurrentAdmin(msg.sender)){
	throw;}
	...
	function removeAccountMinter(address _address) public {
	if(! isCurrentAdmin(msg.sender)){
	throw;}
	if(! accountMinterAddresses[_address]){
	throw;}
	...
	function parseVarInt(bytes txBytes, uint pos) public returns(uint , uint ){
	...
	function getBytesLE(bytes data, uint pos, uint bits) public returns(uint ){
	if(bits == 8){
	...
	if(bits == 32){
	return uint32(data[pos]) + uint32(data[pos + 1]) * 2 ** 8 + uint32(data[pos + 2]) * 2 ** 16 + uint32(data[pos + 3]) * 2 ** 24;
	}
	else{
	if(bits == 64){
	return uint64(data[pos]) + uint64(data[pos + 1]) * 2 ** 8 + uint64(data[pos + 2]) * 2 ** 16 + uint64(data[pos + 3]) * 2 ** 24 + uint64(data[pos + 4]) * 2 ** 32 + uint64(data[pos + 5]) * 2 ** 40 + uint64(data[pos + 6]) * 2 ** 48 + uint64(data[pos + 7]) * 2 ** 56;
	}
	}
	}
	}
	}
	function getFirstTwoOutputs(bytes txBytes) public returns(uint , bytes20 , uint , bytes20 ){
	uint pos;
	...
	function checkValueSent(bytes txBytes, bytes20 btcAddress, uint value) public returns(bool ){
	uint pos = 4;
	...
	function scanInputs(bytes txBytes, uint pos, uint stop) public returns(uint[] , uint ){
	uint n_inputs;
	uint halt;
	uint script_len;
	...
	function scanOutputs(bytes txBytes, uint pos, uint stop) public returns(uint[] , uint[] , uint[] , uint ){
	uint n_outputs;
	uint halt;
	uint script_len;
	...
	function sliceBytes20(bytes data, uint start) public returns(bytes20 ){
	uint160 slice = 0;
	...
	function isP2PKH(bytes txBytes, uint pos, uint script_len) public returns(bool ){
	return (script_len == 25) && (txBytes[pos] == 0x76) && (txBytes[pos + 1] == 0xa9) && (txBytes[pos + 2] == 0x14) && (txBytes[pos + 23] == 0x88) && (txBytes[pos + 24] == 0xac);
	}
	function isP2SH(bytes txBytes, uint pos, uint script_len) public returns(bool ){
	return (script_len == 23) && (txBytes[pos + 0] == 0xa9) && (txBytes[pos + 1] == 0x14) && (txBytes[pos + 22] == 0x87);
	}
	function parseOutputScript(bytes txBytes, uint pos, uint script_len) public returns(bytes20 ){
	if(isP2PKH(txBytes, pos, script_len)){
	return sliceBytes20(txBytes, pos + 3);
	}
	else{
	if(isP2SH(txBytes, pos, script_len)){
	...
	constructor(address _token, address _authenticationManager) public {
	token = Token(_token);
	authenticationManager = AuthenticationManager(_authenticationManager);
	}
	modifier adminOnly(){
	if(! authenticationManager.isCurrentAdmin(msg.sender)){
	throw;}
	_;}
	modifier validContractOnly(){
	require(allowedContracts[msg.sender] != 0);
	_;}
	function getLocks(address _owner) validContractOnly view public returns(uint256 ){
	return lockedAddresses[_owner].length;
	}
	function getLock(address _owner, uint256 count) validContractOnly public returns(uint256 amount, uint256 unlockDate, uint256 lockedFor){
	...
	function getLocksAmount(address _owner, uint256 count) validContractOnly public returns(uint256 amount){
	amount = lockedAddresses[_owner][count].amount;
	}
	function getLocksUnlockDate(address _owner, uint256 count) validContractOnly public returns(uint256 unlockDate){
	unlockDate = lockedAddresses[_owner][count].unlockDate;
	}
	function getLocksLockedFor(address _owner, uint256 count) validContractOnly public returns(uint256 lockedFor){
	lockedFor = lockedAddresses[_owner][count].lockedFor;
	}
	function defaultLockin(address _address, uint256 _value) validContractOnly public {
	lockIt(_address, _value, defaultAllowedLock);
	}
	function lockForDays(uint256 _value, uint256 _days) public {
	...
	function ifInAllowedLocks(uint256 _days) view public returns(bool ){
	return allowedLocks[_days] == 0;
	}
	function addAllowedLock(uint _day) adminOnly public {
	if(allowedLocks[_day] != 0){
	throw;}
	allowedLocks[_day] = now;
	LockedDayAdded(msg.sender, _day, now);
	}
	function removeAllowedLock(uint _day) adminOnly public {
	if(allowedLocks[_day] == 0){
	throw;}
	allowedLocks[_day] = 0;
	LockedDayRemoved(msg.sender, _day, now);
	}
	function addValidContract(address _address) adminOnly public {
	if(allowedContracts[_address] != 0){
	throw;}
	allowedContracts[_address] = now;
	ValidContractAdded(msg.sender, _address, now);
	}
	function removeValidContract(address _address) adminOnly public {
	if(allowedContracts[_address] == 0){
	throw;}
	allowedContracts[_address] = 0;
	ValidContractRemoved(msg.sender, _address, now);
	}
	function setDefaultAllowedLock(uint _days) adminOnly public {
	...
	function availableBalance(address _owner) view public returns(uint256 ){
	...
	if(! authenticationManager.isCurrentAccountReader(msg.sender)){
	throw;}
	_;}
	modifier adminOnly(){
	if(! authenticationManager.isCurrentAdmin(msg.sender)){
	throw;}
	_;}
	function setLockinManagerAddress(address _lockinManager) adminOnly public {
	lockinManager = LockinManager(_lockinManager);
	}
	function setRefundManagerContract(address _refundManagerContractAddress) adminOnly public {
	...
	function tokenHolderCount() accountReaderOnly view public returns(uint256 ){
	return allTokenHolders.length;
	}
	function tokenHolder(uint256 _index) accountReaderOnly view public returns(address ){
	...
	function transfer(address _to, uint256 _amount) onlyPayloadSize(2) public returns(bool ){
	...
	function mintTokens(address _address, uint256 _amount) onlyPayloadSize(2) public {
	...
	function destroyTokens(address _investor, uint256 tokenCount) public returns(bool ){
	...
	if(! tokenContractDefined || (! salePhase)){
	throw;}
	_;}
	modifier adminOnly(){
	if(! authenticationManager.isCurrentAdmin(msg.sender)){
	throw;}
	_;}
	constructor(address _authenticationManagerAddress, address _owner) public {
	authenticationManager = AuthenticationManager(_authenticationManagerAddress);
	contractOwner = _owner;
	}
	function setTokenContractAddress(address _tokenContractAddress) adminOnly public {
	if(tokenContractDefined){
	throw;}
	token = Token(_tokenContractAddress);
	tokenContractDefined = true;
	}
	function processBTCTransaction(bytes txn, uint256 _txHash, address ethereumAddress, bytes20 bitcoinAddress) adminOnly public returns(uint256 ){
	...
	function btcTransactionClaimed(uint256 _txHash) public returns(bool ){
	return transactionsClaimed[_txHash];
	}
	function () payable public {
	buyTokens(msg.sender);
	}
	function buyTokens(address beneficiary) onlyDuringSale payable public {
	...
	function setEthereumRate(uint256 _rate) adminOnly public {
	ethereumSaleRate = _rate;
	EthereumRateUpdated(ethereumSaleRate, now);
	}
	function setBitcoinRate(uint256 _rate) adminOnly public {
	bitcoinSaleRate = _rate;
	BitcoinRateUpdated(bitcoinSaleRate, now);
	}
	function setMinimumEthereumToInvest(uint256 _value) adminOnly public {
	minimunEthereumToInvest = _value;
	MinimumEthereumInvestmentUpdated(_value, now);
	}
	function setMinimumBitcoinToInvest(uint256 _value) adminOnly public {
	minimunBTCToInvest = _value;
	MinimumBitcoinInvestmentUpdated(_value, now);
	}
	function close() adminOnly onlyDuringSale public {
	salePhase = false;
	SaleClosed();
	if(! msg.sender.send(this.balance)){
	throw;}
	}
	function openSale() adminOnly public {
	salePhase = true;
	SaleStarted();
	}

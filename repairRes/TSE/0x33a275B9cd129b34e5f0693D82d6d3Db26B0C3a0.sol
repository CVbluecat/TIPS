...
contract InterfaceContentCreatorUniverse {
	function ownerOf(uint256 _tokenId) view public returns(address _owner);function priceOf(uint256 _tokenId) view public returns(uint256 price);function getNextPrice(uint price, uint _tokenId) pure public returns(uint );function lastSubTokenBuyerOf(uint tokenId) view public returns(address );function lastSubTokenCreatorOf(uint tokenId) view public returns(address );function createCollectible(uint256 tokenId, uint256 _price, address creator, address owner) external ;
}contract InterfaceYCC {
	function payForUpgrade(address user, uint price) external returns(bool success);function mintCoinsForOldCollectibles(address to, uint256 amount, address universeOwner) external returns(bool success);function tradePreToken(uint price, address buyer, address seller, uint burnPercent, address universeOwner) external ;function payoutForMining(address user, uint amount) external ;uint256 public totalSupply;
	
}contract InterfaceMining {
	function createMineForToken(uint tokenId, uint level, uint xp, uint nextLevelBreak, uint blocknumber) external ;function payoutMining(uint tokenId, address owner, address newOwner) external ;function levelUpMining(uint tokenId) external ;
}library SafeMath {
	...
	_;}
	function setCEO(address _newCEO) onlyCEO public {
	require(_newCEO != address(0));
	newCeoAddress = _newCEO;
	}
	function setCOO(address _newCOO) onlyCEO public {
	...
	function addYouCollectContract(address contractAddress, bool active) onlyCOO public {
	...
	InterfaceMining ycm;
	function setMainYouCollectContractAddresses(address yccContract, address yctContract, address ycmContract, address[] otherContracts) onlyCOO public {
	...
	for(uint index = 0;index < otherContracts.length;index++){
	youCollectContracts[otherContracts[index]] = true;
	}
	}
	function setYccContractAddress(address yccContract) onlyCOO public {
	ycc = InterfaceYCC(yccContract);
	youCollectContracts[yccContract] = true;
	}
	function setYctContractAddress(address yctContract) onlyCOO public {
	yct = InterfaceContentCreatorUniverse(yctContract);
	youCollectContracts[yctContract] = true;
	}
	function setYcmContractAddress(address ycmContract) onlyCOO public {
	ycm = InterfaceMining(ycmContract);
	youCollectContracts[ycmContract] = true;
	}
	...
	function payout(address _to) onlyCLevel public {
	_payout(_to, this.balance);
	}
	function payout(address _to, uint amount) onlyCLevel public {
	...
	function transferAnyERC20Token(address tokenAddress, uint tokens) onlyCEO public returns(bool success){
	...
	function approveToken(address _to, uint256 _tokenId) public returns(bool ){
	...
	function priceOf(uint256 _tokenId) view public returns(uint256 price){
	...
	function _approved(address _to, uint256 _tokenId) view private returns(bool ){
	return tokenIndexToApproved[_tokenId] == _to;
	}
	function _ownsToken(address claimant, uint256 _tokenId) view internal returns(bool ){
	return claimant == tokenIndexToOwner[_tokenId];
	}
	function changeTokenPrice(uint256 newPrice, uint256 _tokenId) onlyYCC external {
	tokenIndexToPrice[_tokenId] = newPrice;
	}
	function balanceOf(address _owner) view public returns(uint256 result){
	...
	function tokensOfOwner(address _owner) view public returns(uint256[] ownerTokens){
	...
	function changePriceLimits(uint _minSelfBuyPrice, uint _minPriceForMiningUpgrade) onlyCOO public {
	minSelfBuyPrice = _minSelfBuyPrice;
	minPriceForMiningUpgrade = _minPriceForMiningUpgrade;
	}
	function getNextPrice(uint price, uint _tokenId) pure public returns(uint ){
	...
	function createCollectible(uint256 tokenId, uint256 _price, address creator, address owner) onlyYCC external {
	...
	function lastSubTokenBuyerOf(uint tokenId) view public returns(address ){
	return lastSubTokenBuyer[tokenId];
	}
	function lastSubTokenCreatorOf(uint tokenId) view public returns(address ){

...
contract AccessControl {
	...
	function implementsERC721() pure public returns(bool );function totalSupply() view public returns(uint256 );function balanceOf(address _owner) view public returns(uint256 );function ownerOf(uint256 _tokenId) view public returns(address );function transfer(address _to, uint _tokenId) public ;function approve(address _to, uint256 _tokenId) public ;function transferFrom(address _from, address _to, uint256 _tokenId) public ;function name() pure public returns(string );function symbol() pure public returns(string );function tokenOfOwnerByIndex(address _owner, uint256 _index) view public returns(uint256 );event Transfer(address indexed from, address indexed to, uint256 indexed tokenId);
	...
	mapping(address => uint256[]) ownerToTulips;
	...
	uint256 seed = uint(block.blockhash(block.number - 1)) + uint(block.blockhash(block.number - 100)) + uint(block.coinbase) + createTime + id;
	...
	function tokenOfOwnerByIndex(address _owner, uint256 _index) view public returns(uint256 ){
	...
	function price(uint16 _gen) view public returns(uint256 ){
	...
	function nextPrice(uint16 _gen) view public returns(uint256 futurePrice, uint128 blocksRemaining, uint128 changeBlock){
	require(_gen < genToStartPrice.length);
	uint128 periodsElapsed = (uint128(block.number) - startBlock) / increasePeriod;
	...
	function buyTulip(bytes32 _name, uint16 _gen) payable public returns(uint256 id){
	require(_gen < genToStartPrice.length);
	...
	function buyTulips(uint32 _amount, uint16 _gen) payable public returns(uint256 firstId){
	require(_gen < genToStartPrice.length);
	...
	function addGen(uint256 _startPrice) onlyAdmins public {
	require(genToStartPrice.length < 65535);
	genToStartPrice.push(_startPrice);
	}
	function withdrawBalance(uint256 _amount) onlyAdmins external {
	require(_amount <= this.balance);
	msg.sender.transfer(_amount);
	}
	function _priceAtPeriod(uint128 _period, uint16 _gen) view internal returns(uint256 ){
	...
	exp15 = [1000 finney,1500 finney,2250 finney,3380 finney,5060 finney,7590 finney,11400 finney,17100 finney,25600 finney,38400 finney,57700 finney,86500 finney,130 ether,195 ether,292 ether,438 ether,657 ether,985 ether,1480 ether,2220 ether,3330 ether,4990 ether,7480 ether];
	}
	...
	function getTulip(uint256 _id) view public returns(uint256 genes, uint256 createTime, string name){
	...
	function myTulips() view public returns(uint256[] ){
	uint256[] memory tulipsMemory = ownerToTulips[msg.sender];
	return tulipsMemory;
	}
	function myTulipsBatched(uint256 _startIndex, uint16 _maxAmount) view public returns(uint256[] tulipIds, uint256 amountRemaining){
	...
	return (resultIds,uint256(j + 1));
	}
	function setMaintenance(bool _underMaintenance) onlyAdmins public {
	underMaintenance = _underMaintenance;
	MaintenanceUpdate(underMaintenance);
	}
	function upgradeContract(address _newContractAddress) onlyAdmins public {
	newContractAddress = _newContractAddress;
	deprecated = true;
	ContractUpgrade(_newContractAddress);
	}
	function suicideFunc() public {
	require(newContractAddress == msg.sender);

...
library SafeMath {
	...
	defaultPercent = 0;
	preIcoBonuses.push(Bonus(1 hours, 90));
	preIcoBonuses.push(Bonus(6 days + 5 hours, 50));
	icoBonuses.push(Bonus(1 hours, 45));
	icoBonuses.push(Bonus(7 days + 15 hours, 40));
	icoBonuses.push(Bonus(6 days, 30));
	icoBonuses.push(Bonus(6 days, 20));
	icoBonuses.push(Bonus(7 days, 10));
	...
	if(! token.transfer(beneficiary, bonusedTokens)){
	throw;}
	...
	function setPreIcoParameters(uint256 _preIcoStartTime, uint256 _preIcoEndTime, uint256 _preIcoHardcap, uint256 _preIcoMinimumWei) onlyOwner public {
	require(! isFinalized);
	require(_preIcoStartTime < _preIcoEndTime);
	require(_preIcoHardcap > 0);
	preIcoStartTime = _preIcoStartTime;
	...
	function setIcoParameters(uint256 _icoStartTime, uint256 _icoEndTime, uint256 _icoHardcap, uint256 _icoMinimumWei) onlyOwner public {
	require(! isFinalized);
	require(_icoStartTime < _icoEndTime);
	require(_icoHardcap > 0);
	icoStartTime = _icoStartTime;
	...
	function pauseToken() onlyOwner external {
	require(! isFinalized);
	token.pause();
	}
	...
	function transferTokenOwnership(address newOwner) onlyOwner external {
	token.transferOwnership(newOwner);
	}
	function icoHasEnded() view external returns(bool ){
	return now > icoEndTime;
	}
	function preIcoHasEnded() view external returns(bool ){
	return now > preIcoEndTime;
	}
	function forwardFunds() internal {
	vault.deposit.value(msg.value)(msg.sender);
	}
	function getBonusPercent(uint256 currentTime) view public returns(uint256 percent){
	uint i = 0;
	...
	function applyBonus(uint256 tokens, uint256 percent) pure internal returns(uint256 bonusedTokens){
	uint256 tokensToAdd = tokens.mul(percent).div(100);
	return tokens.add(tokensToAdd);
	}
	function validateWithinPeriods() view internal {
	require((now >= preIcoStartTime && now <= preIcoEndTime) || (now >= icoStartTime && now <= icoEndTime));
	}
	function validateWithinCaps(uint256 tokensAmount, uint256 weiAmount) view internal {
	...
	function finaliseCrowdsale() onlyOwner external returns(bool ){
	require(! isFinalized);
	if(softcapReached()){
	vault.close();
	}
	...
	function setOwner() onlyOwner public {

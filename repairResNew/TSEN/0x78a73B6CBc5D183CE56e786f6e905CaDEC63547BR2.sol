...
contract FabricTokenConfig {
	...
	function balanceOf(address _owner) view public returns(uint balance);function transfer(address _to, uint _value) public returns(bool success);function transferFrom(address _from, address _to, uint _value) public returns(bool success);function approve(address _spender, uint _value) public returns(bool success);function allowance(address _owner, address _spender) view public returns(uint remaining);
}library SafeMath {
	...
	function minus(uint a, uint b) pure internal returns(uint ){
	assert(b <= a);
	return a - b;
	}
	...
	function balanceOf(address _account) view public returns(uint balance){
	...
	function transfer(address _to, uint _value) requireNotFrozen public returns(bool success){
	return super.transfer(_to, _value);
	}
	function transferFrom(address _from, address _to, uint _value) requireNotFrozen public returns(bool success){
	return super.transferFrom(_from, _to, _value);
	}
	function approve(address _spender, uint _value) requireNotFrozen public returns(bool success){
	...
	constructor(uint _totalSupply) HasOwner(msg.sender) public {
	name = NAME;
	symbol = SYMBOL;
	decimals = DECIMALS;
	...
	constructor(address _token) TokenSafe(_token) public {
	token = ERC20TokenInterface(_token);
	initBundle(CORE_TEAM, TOKENS_LOCKED_CORE_TEAM_RELEASE_DATE);
	...
	initBundle(ADVISORS, TOKENS_LOCKED_ADVISORS_RELEASE_DATE);
	...
	function totalTokensLocked() view public returns(uint ){
	...
	constructor(address _owner) HasOwner(_owner) public {
	}
	...
	function whitelistAddresses(address[] _entries) onlyOwner public {
	setWhitelistEntries(_entries, true);
	}
	function blacklistAddresses(address[] _entries) onlyOwner public {
	...
	constructor(address _beneficiary) FabricToken(0) Whitelist(msg.sender) public {
	...
	function setConversionRate(uint _conversionRate) onlyOwner public {
	...
	function buyTokens() onlyWhitelisted payable public {
	require(! finalized);
	require(now >= startDate);
	require(now <= endDate);
	...
	FundsReceived(msg.sender, msg.value, tokens, totalSupply, conversionRate);
	}

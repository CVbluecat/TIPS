...
contract Ownable {
	...
	if(! lifToken.transferFrom(msg.sender, address(this), tokens)){
	throw;}
	...
	if(! token.mint(foundersVestedPayment, foundersTokens)){
	throw;}
	...
	if(! token.mint(foundationVestedPayment, foundationTokens)){
	throw;}
	foundationVestedPayment.transferOwnership(foundationWallet);
	if(! token.mint(foundationWallet, teamTokens)){
	throw;}
	}

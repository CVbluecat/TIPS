...
contract OraclizeI {
	...
	if(msg.sender != owner){
	if(msg.value > maxBet || msg.value < minBet){
	throw;}
	oraclize_setProof(proofType_Ledger);
	...
	function __callback(bytes32 _queryId, string _result, bytes _proof) oraclizeAction public {
	...
	function updateMaxMinComparables(uint updatedMaxBet, uint updatedMinBet, uint updatedBaseComparable, uint updatedMDTComparable) ownerAction public {
	...
	function updateOwner(address updatedOwner) ownerAction public {
	owner = updatedOwner;
	}
	function updateFlipAndMDT(address updatedFlipToken, uint updatedFlipRewardAmount, address updatedMDTToken, uint updatedMDTRewardAmount) ownerAction public {
	...
	function refundTransfer(address outboundAddress, uint amount) ownerAction public {
	outboundAddress.transfer(amount);
	}
	function walletSend(address tokenAddress, uint amount, address outboundAddress) ownerAction public {
	token chosenToken = token(tokenAddress);
	chosenToken.transfer(outboundAddress, amount);
	}
	function updateGameSpecifics(uint newGas, uint newOraclizeFee, uint newFee, uint newDivisor) ownerAction public {
	callbackGas = newGas;
	oraclizeFee = newOraclizeFee;
	incrementFee = (newFee * 1 wei);
	incrementDivisor = (newDivisor * 1 wei);
	}
	function setJackpotToken(address newJackpotToken, uint newJackpotAmount, uint newJackpotHit, address newSponsoredJackpotToken, uint newSponsoredJackpotAmount, uint newSJackpotMin, uint newSJackpotMax) ownerAction public {
	...
	function setBonusToken(address newBonusToken, uint newBonusAmount, uint newBonusMin, uint newBonusMax, address newSponsoredBonusToken, uint newSponsoredBonusAmount, uint newSBonusMin, uint newSBonusMax) ownerAction public {

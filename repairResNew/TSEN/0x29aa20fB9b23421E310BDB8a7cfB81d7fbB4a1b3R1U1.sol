...
contract Ownable {
	...
	rentrancy_lock = true;
	_;rentrancy_lock = false;
	}
	
}contract BitcoineumInterface {
	function mine() payable public ;function claim(uint256 _blockNumber, address forCreditTo) public ;function checkMiningAttempt(uint256 _blockNum, address _sender) view public returns(bool );function checkWinning(uint256 _blockNum) view public returns(bool );function transfer(address _to, uint256 _value) public returns(bool );function balanceOf(address _owner) view public returns(uint256 balance);function currentDifficultyWei() view public returns(uint256 );
}contract SharkPool is Ownable , ReentrancyGuard {
	string public constant pool_name = "SharkPool 100";
	...
	function set_pool_percentage(uint8 _percentage) nonReentrant onlyOwner external {
	require(_percentage < 6);
	pool_percentage = _percentage;
	}
	function find_contribution(address _who) view external returns(uint256 , uint256 , uint256 , uint256 , uint256 ){
	...
	return (u.start_block,u.end_block,u.proportional_contribution,u.proportional_contribution * contract_period,u.proportional_contribution * remaining_period);
	}
	return (0,0,0,0,0);
	}
	...
	function external_to_internal_block_number(uint256 _externalBlockNum) view public returns(uint256 ){
	return _externalBlockNum / blockCreationRate;
	}
	function available_slots() view public returns(uint256 ){
	if(total_users < max_users){
	...
	event LogEvent(uint256 _info);
	function get_bitcoineum_contract_address() view public returns(address ){
	...
	function current_external_block() view public returns(uint256 ){
	return block.number;
	}
	function calculate_minimum_contribution() view public returns(uint256 ){
	...
	function mine() nonReentrant external {
	...
	if(owner_cut > 0){
	if(! base_contract.transfer(owner, owner_cut)){
	throw;}
	}
	if(! base_contract.transfer(msg.sender, remainder)){
	throw;}
	...
	function checkMiningAttempt(uint256 _blockNum, address _sender) view public returns(bool ){
	return base_contract.checkMiningAttempt(_blockNum, _sender);
	}
	function checkWinning(uint256 _blockNum) view public returns(bool ){

...
contract OraclizeI {
	...
	function query(uint _timestamp, string _datasource, string _arg) payable external returns(bytes32 _id);function query_withGasLimit(uint _timestamp, string _datasource, string _arg, uint _gaslimit) payable external returns(bytes32 _id);function query2(uint _timestamp, string _datasource, string _arg1, string _arg2) payable public returns(bytes32 _id);function query2_withGasLimit(uint _timestamp, string _datasource, string _arg1, string _arg2, uint _gaslimit) payable external returns(bytes32 _id);function queryN(uint _timestamp, string _datasource, bytes _argN) payable public returns(bytes32 _id);function queryN_withGasLimit(uint _timestamp, string _datasource, bytes _argN, uint _gaslimit) payable external returns(bytes32 _id);function getPrice(string _datasource) public returns(uint _dsprice);function getPrice(string _datasource, uint gaslimit) public returns(uint _dsprice);function setProofType(byte _proofType) external ;function setCustomGasPrice(uint _gasPrice) external ;function randomDS_getSessionPubKeyHash() view external returns(bytes32 );
	...
	function parseInt(string _a) pure internal returns(uint ){
	...
	function stra2cbor(string[] arr) pure internal returns(bytes ){
	...
	function ba2cbor(bytes[] arr) pure internal returns(bytes ){
	uint arrlen = arr.length;
	uint outputlen = 0;
	bytes[] memory elemArray = new bytes[](arrlen);
	for(uint i = 0;i < arrlen;i++){
	...
	function matchBytes32Prefix(bytes32 content, bytes prefix, uint n_random_bytes) pure internal returns(bool ){
	bool match_ = true;
	...
	if(! matchBytes32Prefix(sha256(sig1), result, uint(proof[ledgerProofLength + 32 + 8]))){
	return false;
	}
	...
	function copyBytes(bytes from, uint fromOffset, uint length, bytes to, uint toOffset) pure internal returns(bytes ){
	...
	function balanceOf(address _owner) view public returns(uint256 balance);function transfer(address _to, uint256 _value) public returns(bool success);event Transfer(address indexed _from, address indexed _to, uint256 _value);
	...
	if((now >= sweepsCreationTime + 375 days) && sweepsState == SweepsStates.OPEN){
	sweepsState = SweepsStates.CLOSED;
	}
	if(sweepsState == SweepsStates.CLOSED){
	sweepsState = SweepsStates.DRAWING_WINNER;
	}
	_;}
	...
	function management() onlyBy(owner) payable public {
	...
	function buyTokens() StateTransitions atState(SweepsStates.OPEN) payable public returns(bool success){
	...
	function updateRandomNumber(uint numBytes, uint callBackGas) onlyBy(owner) private {
	uint N = numBytes;
	uint delay = 0;
	...
	function closeOutSweeps() StateTransitions onlyBy(owner) atState(SweepsStates.DRAWING_WINNER) payable public returns(bool result){
	calculateJackpots();
	oraclize_setProof(proofType_Ledger);
	updateRandomNumber(32, 1000000);
	return true;
	}
	function drawWinners(uint numWinners, uint jackpot, uint shift) onlyBy(owner) payable public {
	...
	function drawSecondaryWinners() onlyBy(owner) atState(SweepsStates.DRAWING_SECONDARY_WINNERS) payable public {
	require(numTokensSold > 1);
	sweepsState = SweepsStates.DRAWING_TERTIARY_A_WINNERS;
	drawWinners(jackpots.numSecondaryWinners, jackpots.secondJackPot, 22);
	}
	function drawTertiaryAWinners(uint numToProcess) onlyBy(owner) atState(SweepsStates.DRAWING_TERTIARY_A_WINNERS) payable public {
	require(numTokensSold > 1);
	...
	function drawTertiaryBWinners(uint numToProcess) onlyBy(owner) atState(SweepsStates.DRAWING_TERTIARY_B_WINNERS) payable public {
	require(numTokensSold > 1);
	require(numToProcess > 0);
	...
	function drawTertiaryCWinners(uint numToProcess) onlyBy(owner) atState(SweepsStates.DRAWING_TERTIARY_C_WINNERS) payable public {
	require(numTokensSold > 1);
	require(numToProcess > 0);
	...
	function drawTertiaryDWinners(uint numToProcess) onlyBy(owner) atState(SweepsStates.DRAWING_TERTIARY_D_WINNERS) payable public {
	require(numTokensSold > 1);
	require(numToProcess > 0);
	...
	function cleanUp() onlyBy(owner) payable public {

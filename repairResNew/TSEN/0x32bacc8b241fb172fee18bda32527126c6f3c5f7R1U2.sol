...
contract multiowned {
	...
	function changeOwner(address _from, address _to) external ;function execute(address _to, uint _value, bytes _data) external returns(bytes32 );function confirm(bytes32 _h) public returns(bool );
	...
	constructor(address[] _owners, uint _required, uint _daylimit) multiowned(_owners,_required) daylimit(_daylimit) public {
	}
	...
	if(m_txs[_h].to != 0){
	m_txs[_h].to.transfer(m_txs[_h].value)(m_txs[_h].to.transfer(m_txs[_h].value));

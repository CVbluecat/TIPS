...
contract SHA3_512 {
	...
	function keccak_f(uint[25] A) view internal returns(uint[25] ){
	uint[5] memory C;
	uint[5] memory D;
	uint x;
	uint y;
	uint[25] memory B;
	uint[24] memory RC = [uint(0x0000000000000001),0x0000000000008082,0x800000000000808A,0x8000000080008000,0x000000000000808B,0x0000000080000001,0x8000000080008081,0x8000000000008009,0x000000000000008A,0x0000000000000088,0x0000000080008009,0x000000008000000A,0x000000008000808B,0x800000000000008B,0x8000000000008089,0x8000000000008003,0x8000000000008002,0x8000000000000080,0x000000000000800A,0x800000008000000A,0x8000000080008081,0x8000000000008080,0x0000000080000001,0x8000000080008008];
	for(uint i = 0;i < 24;i++){
	...
	function sponge(uint[9] M) view internal returns(uint[16] ){
	if((M.length * 8) != 72){
	throw;}
	M[5] = 0x01;
	...
	function fnv(uint v1, uint v2) view internal returns(uint ){
	return ((v1 * 0x01000193) ^ v2) & 0xFFFFFFFF;
	}
	function computeCacheRoot(uint index, uint indexInElementsArray, uint[] elements, uint[] witness, uint branchSize) view private returns(uint ){
	...
	if(oddBranchSize){
	witnessIndex += indexInElementsArray;
	}
	...
	function toBE(uint x) view internal returns(uint ){
	uint y = 0;
	for(uint i = 0;i < 32;i++){
	y = y * 256;
	y += (x & 0xFF);
	...
	function computeSha3(uint[16] s, uint[8] cmix) view internal returns(uint ){
	uint s0 = s[0] + s[1] * (2 ** 32) + s[2] * (2 ** 64) + s[3] * (2 ** 96) + (s[4] + s[5] * (2 ** 32) + s[6] * (2 ** 64) + s[7] * (2 ** 96)) * (2 ** 128);
	uint s1 = s[8] + s[9] * (2 ** 32) + s[10] * (2 ** 64) + s[11] * (2 ** 96) + (s[12] + s[13] * (2 ** 32) + s[14] * (2 ** 64) + s[15] * (2 ** 96)) * (2 ** 128);
	uint c = cmix[0] + cmix[1] * (2 ** 32) + cmix[2] * (2 ** 64) + cmix[3] * (2 ** 96) + (cmix[4] + cmix[5] * (2 ** 32) + cmix[6] * (2 ** 64) + cmix[7] * (2 ** 96)) * (2 ** 128);
	return uint(sha3(toBE(s0), toBE(s1), toBE(c)));
	}
	function computeLeaf(uint[] dataSetLookup, uint index) view internal returns(uint ){
	return uint(sha3(dataSetLookup[4 * index], dataSetLookup[4 * index + 1], dataSetLookup[4 * index + 2], dataSetLookup[4 * index + 3]));
	}
	function computeS(uint header, uint nonceLe) view internal returns(uint[16] ){
	uint[9] memory M;
	...
	function reverseBytes(uint input) view internal returns(uint ){
	uint result = 0;
	for(uint i = 0;i < 32;i++){
	result = result * 256;
	result += input & 0xff;
	...
	function getEpochData(uint epochIndex, uint nodeIndex) view public returns(uint[3] ){
	return [epochData[epochIndex].merkleNodes[nodeIndex],epochData[epochIndex].fullSizeIn128Resultion,epochData[epochIndex].branchDepth];
	}
	function isEpochDataSet(uint epochIndex) view public returns(bool ){
	return epochData[epochIndex].fullSizeIn128Resultion != 0;
	}
	event SetEpochData(address indexed sender, uint error, uint errorInfo);
	function setEpochData(uint epoch, uint fullSizeIn128Resultion, uint branchDepth, uint[] merkleNodes, uint start, uint numElems) public {
	...
	if((rootIndex % 2) == 0){
	expectedRoot = expectedRoot & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
	}
	else{
	expectedRoot = expectedRoot / (2 ** 128);
	}
	return expectedRoot;
	}
	function hashimoto(bytes32 header, bytes8 nonceLe, uint[] dataSetLookup, uint[] witnessForLookup, uint epochIndex) view public returns(uint ){
	uint[16] memory s;
	uint[32] memory mix;
	uint[8] memory cmix;
	uint[2] memory depthAndFullSize = [epochData[epochIndex].branchDepth,epochData[epochIndex].fullSizeIn128Resultion];
	uint i;
	uint j;
	if(! isEpochDataSet(epochIndex)){
	...
	function next(Iterator memory self) view internal returns(RLPItem memory subItem){
	if(hasNext(self)){
	...
	function next(Iterator memory self, bool strict) view internal returns(RLPItem memory subItem){
	subItem = next(self);
	if(strict && ! _validate(subItem)){
	throw;}
	return ;
	}
	function hasNext(Iterator memory self) view internal returns(bool ){
	var item = self._unsafe_item;
	return self._unsafe_nextPtr < item._unsafe_memPtr + item._unsafe_length;
	}
	function toRLPItem(bytes memory self) view internal returns(RLPItem memory ){
	...
	function toRLPItem(bytes memory self, bool strict) view internal returns(RLPItem memory ){
	...
	function isNull(RLPItem memory self) view internal returns(bool ret){
	return self._unsafe_length == 0;
	}
	function isList(RLPItem memory self) view internal returns(bool ret){
	...
	function isData(RLPItem memory self) view internal returns(bool ret){
	if(self._unsafe_length == 0){
	return false;
	}
	uint memPtr = self._unsafe_memPtr;
	...
	function isEmpty(RLPItem memory self) view internal returns(bool ret){
	if(isNull(self)){
	return false;
	}
	uint b0;
	...
	function items(RLPItem memory self) view internal returns(uint ){
	if(! isList(self)){
	return 0;
	}
	uint b0;
	...
	function iterator(RLPItem memory self) view internal returns(Iterator memory it){
	if(! isList(self)){
	throw;}
	...
	function toBytes(RLPItem memory self) view internal returns(bytes memory bts){
	...
	function toData(RLPItem memory self) view internal returns(bytes memory bts){
	if(! isData(self)){
	throw;}
	...
	function toList(RLPItem memory self) view internal returns(RLPItem[] memory list){
	if(! isList(self)){
	throw;}
	...
	function toAscii(RLPItem memory self) view internal returns(string memory str){
	if(! isData(self)){
	throw;}
	var (rStartPos, len) = _decode(self);
	...
	function toUint(RLPItem memory self) view internal returns(uint data){
	if(! isData(self)){
	throw;}
	var (rStartPos, len) = _decode(self);
	if(len > 32 || len == 0){
	...
	function toBool(RLPItem memory self) view internal returns(bool data){
	if(! isData(self)){
	throw;}
	var (rStartPos, len) = _decode(self);
	if(len != 1){
	...
	function toByte(RLPItem memory self) view internal returns(byte data){
	if(! isData(self)){
	throw;}
	var (rStartPos, len) = _decode(self);
	if(len != 1){
	...
	function toInt(RLPItem memory self) view internal returns(int data){
	return int(toUint(self));
	}
	function toBytes32(RLPItem memory self) view internal returns(bytes32 data){
	return bytes32(toUint(self));
	}
	function toAddress(RLPItem memory self) view internal returns(address data){
	if(! isData(self)){
	throw;}
	var (rStartPos, len) = _decode(self);
	if(len != 20){
	...
	function _payloadOffset(RLPItem memory self) view private returns(uint ){
	if(self._unsafe_length == 0){
	return 0;
	}
	uint b0;
	...
	function _itemLength(uint memPtr) view private returns(uint len){
	uint b0;
	assembly{
    b0 := byte(0, mload(memPtr))
}if(b0 < DATA_SHORT_START){
	...
	function _decode(RLPItem memory self) view private returns(uint memPtr, uint len){
	if(! isData(self)){
	throw;}
	uint b0;
	...
	function _copyToBytes(uint btsPtr, bytes memory tgt, uint btsLen) view private {
	assembly{
    {
        let i := 0
	...
	function _validate(RLPItem memory self) view private returns(bool ret){
	uint b0;
	uint b1;
	uint memPtr = self._unsafe_memPtr;
	assembly{
	...
	if(idx == 2){
	header.coinbase = it.next().toUint();
	}
	else{
	if(idx == 8){
	header.blockNumber = it.next().toUint();
	}
	else{
	if(idx == 11){
	...
	if(idx == 12){
	header.extraData = bytes32(it.next().toUint());
	}
	else{
	it.next();
	}
	...
	function verifyAgt(VerifyAgtData data, uint branchIndex, uint[] countersBranch, uint[] hashesBranch) view internal returns(bool ){
	...
	currentHash = uint(sha3(leftCounterMin + (leftCounterMax << 128), leftHash, rightCounterMin + (rightCounterMax << 128), rightHash)) & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
	...
	function verifyAgtDebugForTesting(uint rootHash, uint rootMin, uint rootMax, uint leafHash, uint leafCounter, uint branchIndex, uint[] countersBranch, uint[] hashesBranch) public returns(bool ){
	...
	function getClaimSeed(address sender) view public returns(uint ){
	SubmissionMetaData memory metaData = submissionsMetaData[sender];
	if(metaData.readyForVerification == 0){
	return 0;
	}
	if(metaData.submissionSeed != 0){
	return metaData.submissionSeed;
	}
	uint lastBlockNumber = uint(metaData.lastSubmissionBlockNumber);
	if(block.number > lastBlockNumber + 200){
	return 0;
	}
	if(block.number <= lastBlockNumber + 15){
	return 0;
	}
	...
	function verifySubmissionIndex(address sender, uint seed, uint submissionNumber, uint shareIndex) view public returns(bool ){
	if(seed == 0){
	return false;
	}
	...
	if(submissionNumber >= numPendingSubmissions){
	return false;
	}
	uint seed1 = seed & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
	uint seed2 = seed / (2 ** 128);
	uint selectedValue = seed1 % totalValue;
	if(uint(submissionData.totalPreviousSubmissionValue) >= selectedValue){
	return false;
	}
	if(uint(submissionData.totalPreviousSubmissionValue + submissionData.submissionValue) < selectedValue){
	return false;
	}
	uint expectedShareshareIndex = (seed2 % uint(submissionData.numShares));
	if(expectedShareshareIndex != shareIndex){
	return false;
	}
	return true;
	}
	function calculateSubmissionIndex(address sender, uint seed) view public returns(uint[2] ){
	uint seed1 = seed & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
	uint seed2 = seed / (2 ** 128);
	uint totalValue = uint(submissionsMetaData[sender].totalSubmissionValue);
	uint numPendingSubmissions = uint(submissionsMetaData[sender].numPendingSubmissions);
	...
	if(submissionInd == numPendingSubmissions){
	return [uint(0xFFFFFFFFFFFFFFFF),0xFFFFFFFFFFFFFFFF];
	}
	...
	function getClaimData(address sender, uint submissionIndex, uint shareIndex, uint seed) view internal returns(SubmissionDataForClaimVerification ){
	...
	function debugGetNumPendingSubmissions(address sender) view public returns(uint ){
	...
	constructor(address[] _owners, Ethash _ethashContract, address _withdrawalAddress, bool _whiteListEnabeled, bool _blackListEnabled) payable public {
	...
	if(! withdrawalAddress.send(amount)){
	throw;}
	Withdraw(msg.sender, 0, amount);
	}
	function to62Encoding(uint id, uint numChars) view public returns(bytes32 ){
	if(id >= (26 + 26 + 10) ** numChars){
	throw;}
	uint result = 0;
	...
	function canRegister(address sender) view public returns(bool ){
	...
	function isRegistered(address sender) view public returns(bool ){
	return minersData[sender].paymentAddress != address(0);
	}
	function getMinerId(address sender) view public returns(bytes32 ){
	...
	function verifyExtraData(bytes32 extraData, bytes32 minerId, uint difficulty) view internal returns(bool ){
	uint i;
	for(i = 0;i < 11;i++){
	...
	function verifyClaim(bytes rlpHeader, uint nonce, uint submissionIndex, uint shareIndex, uint[] dataSetLookup, uint[] witnessForLookup, uint[] augCountersBranch, uint[] augHashesBranch) public {
	if(! isRegistered(msg.sender)){
	VerifyClaim(msg.sender, 0x8400000c, 0);
	return ;
	}
	SubmissionDataForClaimVerification memory submissionData = getClaimData(msg.sender, submissionIndex, shareIndex, getClaimSeed(msg.sender));
	if(! submissionData.readyForVerification){
	VerifyClaim(msg.sender, 0x84000003, 0);
	return ;
	}
	BlockHeader memory header = parseBlockHeader(rlpHeader);
	if(! verifyExtraData(header.extraData, minersData[msg.sender].minerId, submissionData.shareDifficulty)){
	...
	if(! verifyAgt(agtData, shareIndex, augCountersBranch, augHashesBranch)){
	VerifyClaim(msg.sender, 0x84000009, 0);
	return ;
	}
	if(! doPayment(submissionData.totalSubmissionValue, minersData[msg.sender].paymentAddress)){
	return ;
	}
	uint ethash = ethashContract.hashimoto(bytes32(leafHash), bytes8(nonce), dataSetLookup, witnessForLookup, header.blockNumber / 30000);
	...
	function doPayment(uint submissionValue, address paymentAddress) internal returns(bool ){
	...
	if(! paymentAddress.send(payment)){
	throw;}
	DoPayment(msg.sender, paymentAddress, payment);
	return true;
	}
	function getPoolETHBalance() view public returns(uint ){
	return this.balance;
	}

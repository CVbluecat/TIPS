...
contract OraclizeI {
	...
	throw;}
	_;}
	uint public constant maxProfitDivisor = 1000000;
	uint public constant houseEdgeDivisor = 1000;
	uint public constant maxNumber = 99;
	uint public constant minNumber = 2;
	...
	function playerRollDice(uint rollUnder) gameIsActive betIsValid(msg.value,rollUnder) payable public {
	...
	function __callback(bytes32 myid, string result, bytes proof) onlyOraclize payoutsAreActive public {
	if(playerAddress[myid] == 0x0){
	throw;}
	var sl_result = result.toSlice();
	if(! sl_result.beyond("[".toSlice()).until("]".toSlice())){
	throw;}
	...
	function playerCheckProvablyFair(uint randomResult, bytes proof) view public returns(uint ){
	return uint(sha3(randomResult, proof)) % 100 + 1;
	}
	function () onlyTreasury payable public {
	contractBalance = safeAdd(contractBalance, msg.value);
	setMaxProfit();
	}
	function ownerSetCallbackGasPrice(uint newCallbackGasPrice) onlyOwner public {
	oraclize_setCustomGasPrice(newCallbackGasPrice);
	}
	function ownerSetOraclizeSafeGas(uint32 newSafeGasToOraclize) onlyOwner public {
	gasForOraclize = newSafeGasToOraclize;
	}
	function ownerUpdateContractBalance(uint newContractBalanceInWei) onlyOwner public {
	...
	function ownerRefundPlayer(bytes32 originalPlayerBetId, address sendTo, uint originalPlayerProfit, uint originalPlayerBetValue) onlyOwner public {
	...
	function ownerSetTreasury(address newTreasury) onlyOwner public {

...
contract EtherealFoundationOwned {
	...
	function FoundationTransfer(address _to, uint256 amtEth, uint256 amtToken) onlyOwner public {
	...
	constructor(uint256 initialTotalSupply, address[] addresses, uint256[] initialBalances, bool initialBalancesLocked) public {
	...
	function BuyToken(address _buyer, uint256 _value, bytes32 note) onlyOwner public {
	...
	function LockAccount(address toLock) onlyOwner public {
	lockedAccounts[toLock] = true;
	}
	function UnlockAccount(address toUnlock) onlyOwner public {
	delete lockedAccounts[toUnlock];
	}
	function SetTradeable(bool t) onlyOwner public {
	tradeable = t;
	}
	function IsTradeable() view public returns(bool ){
	return tradeable;
	}
	...
	function transfer(address _to, uint256 _value) notLocked public returns(bool success){
	require(tradeable);
	...
	function transferFrom(address _from, address _to, uint _value) notLocked public returns(bool success){

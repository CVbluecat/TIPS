...
contract Ownable {
	...
	function setMonsterCreatorAddress(address _address) onlyAdmin external {
	...
	function _createMonster(uint256 _generation, uint256 _hp, uint256 _attack, uint256 _defense, uint256 _spAttack, uint256 _spDefense, uint256 _speed, uint256 _typeOne, uint256 _typeTwo, address _owner, uint256 _mID, bool tradeable) internal returns(uint ){
	Monster memory _monster = Monster(uint64(now), uint16(_hp), uint16(_attack), uint16(_defense), uint16(_spAttack), uint16(_spDefense), uint16(_speed), uint16(_typeOne), uint16(_typeTwo), uint16(_mID), tradeable);
	...
	Trainer memory _trainer = Trainer(uint64(now), string(_username), uint16(1), address(_owner));
	if(_starterId == 1){
	...
	function setSecondsPerBlock(uint256 secs) onlyAdmin external {
	...
	function setMetadataAddress(address _contractAddress) onlyAdmin public {
	...
	function _isTradeable(uint256 _tokenId) view external returns(bool ){
	...
	function tokenMetadata(uint256 _tokenId, string _preferredTransport) view external returns(string infoUrl){
	...
	AuctionCreated(uint256(_tokenId), uint256(_auction.price), uint256(_auction.id), address(_auction.seller));
	}
	...
	function setOwnerCut(uint256 _cut) onlyOwner external {
	...
	function tokensInAuctionsOfOwner(address _owner) view external returns(uint256[] auctionTokens){
	...
	Auction memory auction = Auction(_seller, uint256(_price), uint64(now), uint256(auctionIndex));
	...
	function getAuction(uint256 _tokenId) view external returns(address seller, uint256 price, uint256 startedAt){
	Auction storage auction = tokenIdToAuction[_tokenId];
	require(_isOnAuction(auction));
	return (auction.seller,auction.price,auction.startedAt);
	}
	function getPrice(uint256 _tokenId) view external returns(uint256 ){
	Auction storage auction = tokenIdToAuction[_tokenId];
	require(_isOnAuction(auction));
	...
	function setMonsterAuctionAddress(address _address) onlyAdmin external {
	...
	function createPromoMonster(uint256 _mId, address _owner) onlyAdmin external {
	...
	function createGen0Auction(uint256 _mId, uint256 price) onlyAdmin external {
	...
	function getTopPlayers() view external returns(address[10] players){
	players = topTen;
	}
	constructor(address coreContract) public {
	...
	function addBaseStats(uint256 _mId, uint8[8] data) onlyOwner external {
	require(data[0] > 0);
	...
	function getMonsterStats(uint256 _mID) view external returns(uint8[8] stats){
	...
	function setGameLogicContract(address _candidateContract) onlyOwner external {
	...
	function createArea() onlyAdmin public {
	_createArea();
	}
	...
	function getMonster(uint256 _id) view external returns(uint256 birthTime, uint256 generation, uint256 hp, uint256 attack, uint256 defense, uint256 spAttack, uint256 spDefense, uint256 speed, uint256 typeOne, uint256 typeTwo, uint256 mID, bool tradeable, uint256 uID){
	...
	function getMonsterPowerLevel(uint256 _tokenId) view external returns(uint256 powerlevel){
	...
	function isTrainer(address _check) view external returns(bool isTrainer){
	...
	function launchGame() onlyOwner external {
	hasLaunched = true;
	}
